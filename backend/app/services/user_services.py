...
class UserService:
    ...
    async def change_user_data(self, *, 
                               user_id: int, 
                               new_data: UserUpdateSchema
                               ) -> UserBasicSchema:
        new_user = await self.user_repo.update(db=self.db,
    ...
    async def get_user_by_id_basic(self, 
                             user_id: int
                             ) -> UserBasicSchema:
        user = await self.user_repo.get_by_id_or_none(db=self.db, 
                                                      id=user_id,)
        if user is None:
            raise NotFoundException("User doesn't exist")
        return UserBasicSchema.model_validate(user)
    
    async def get_user_by_id_with_books(self,
                                         user_id: int
                                         ) -> UserWithBooksSchema:
        user = await self.user_repo.get_by_id_or_none(db=self.db,
                                                      id=user_id,
    ...
    async def get_user_by_id_with_notes(self,
                                         user_id: int) -> UserWithNotesSchema:
        user = await self.user_repo.get_by_id_or_none(db=self.db,
                                                      id=user_id,
                                                      include_notes=True)
        if user is None:
    ...
    async def update(self, *, uid: int, 
                     user_data: UserUpdateSchema) -> UserBasicSchema:
        updated_user = await self.user_repo.update(db=self.db, 
                                                    id=uid, 
