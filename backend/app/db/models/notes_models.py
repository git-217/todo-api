...
class Note(BaseSAModel):
    ...
    status: Mapped[CompleteStatus] = mapped_column(
        default=CompleteStatus.IN_PROGRESS, server_default=text("'in progress'")
    )
    ...
    # 1:N book:notes
    book: Mapped["Book"] = relationship("Book", uselist=False, back_populates="notes")
    # 1:N user:notes
    user: Mapped["User"] = relationship("User", uselist=False, back_populates="notes")
