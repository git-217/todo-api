...
class BookCRUDRepo(CRUDBase[Book, BookCreateSchema, BookUpdateSchema]):
    ...
    async def get_book_with_notes(self, *, db: AsyncSession, book_id: int) -> Book:
    ...
    async def autochange_book_stat(self, db: AsyncSession, book_id: int):
        ...
        unfinished_notes = sum(
            1 for note in book.notes if note.status != CompleteStatus.COMPLETED
        )
