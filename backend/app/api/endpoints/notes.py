...
from app.schemas.notes_schema import NoteCreateSchema, NoteReadSchema, NoteUpdateSchema
from app.schemas.response_schema import (
    create_response,
    GetResponseBase,
    PostResponseBase,
...
async def create_new_note(
    ...
    new_note = await note_service.create_note(
        owner=user, book_id=book_id, note_data=note_data
    )
...
async def get_book_note(
    ...
    note = await note_service.get_one(owner=user, book_id=book_id, note_id=note_id)
...
async def change_note_data(
    ...
    note = await note_service.update(
        owner=user, book_id=book_id, note_id=note_id, note_data=note_data
    )
    return create_response(data=note)
...
async def delete_note(
    ...
    deleted_note = await note_service.delete(
        owner=user, book_id=book_id, note_id=note_id
    )
